...
namespace invoicing.Transactions
{
    ...
    /// 出貨單表單
    /// </summary>
    public partial class SalesDeliveryForm : Form
    {
        ...
        /// 是否為關貿格式（日期顯示為空白格式）
        /// </summary>
        private bool _isCustomsFormat = false;
        ...
        /// 單子類型名稱
        /// </summary>
        private const string OrderType = "出貨單";

        public SalesDeliveryForm()
        {
            InitializeComponent();
        }
        ...
        private void BtnLoad_Click(object? sender, EventArgs e)
        {
            try
            {
                _transactionsbtnService.OpenReadInvoicesForm(nameof(SalesDeliveryForm), OrderType);
            }
            catch (Exception ex)
            {
        ...
        private async void BtnPrint_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                var request = new PrintInvoiceRequest
                {
                    ...
                    // 關貿格式使用空白日期
                    Date = _isCustomsFormat ? "    年   月   日" : dtpDate.Text,
        ...
        {
            if (e.CallerFormType != nameof(SalesDeliveryForm))
            {
                return;
            }
