...
namespace invoicing.Transactions
{
    ...
    public partial class PurchaseReturnForm : Form
    {
        ...
        /// 單子類型名稱
        /// </summary>
        private const string OrderType = "進貨退出單";

        /// <summary>
        /// 特殊廠商名稱，需要進行價格除以1.05處理
        /// </summary>
        private const string SpecialSupplierName = "唐詣";
        ...
        private void RegisterEventHandlers()
        {
            ...
            // 業務邏輯事件（使用自訂的 CellEndEdit 處理器，處理特殊廠商價格邏輯）
            dgvInvoicing.CellEndEdit += async (sender, e) =>
            {
                await HandleCellEndEditAsync(e);
            };
        ...
        /// <summary>
        /// 自訂的 CellEndEdit 處理器，處理特殊廠商（唐詣）的價格除以1.05邏輯
        /// </summary>
        private async Task HandleCellEndEditAsync(DataGridViewCellEventArgs e)
        {
            ...
            string columnName = dgvInvoicing.Columns[e.ColumnIndex].HeaderCell.Value?.ToString() ?? "";
            ...
            else if (columnName == "數量" || columnName == "單價")
            {
                _transactionsdgvService.CalculateRowAmount(row);
                double total = _transactionsdgvService.CalculateTotalAmount(dgvInvoicing);
                UpdateTotalAmountLabel(total);
            }
        }

        /// <summary>
        /// 根據產品編號取得產品資訊並填入 DataGridView 行（含特殊廠商價格處理）
        /// 對於「唐詣」廠商，單價需除以1.05
        /// </summary>
        private async Task<bool> FetchProductInfoWithSpecialPriceAsync(DataGridViewRow row, string productCode)
        {
            ...
            try
            {
                ...
                if (product != null)
                {
                    ...
                    row.Cells["UnitPrice"].Value = unitPrice.ToString("0.##");
                    return true;
                }
        ...
        private void BtnLoad_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                _transactionsbtnService.OpenReadInvoicesForm(nameof(PurchaseReturnForm), OrderType);
            }
            catch (Exception ex)
            {
        ...
        {
            // 確認事件來源是本表單
            if (e.CallerFormType != nameof(PurchaseReturnForm))
            {
                return;
            }
